  ...
  <div class="param-alarm">
    ...
    <a-table :columns="columns" :dataSource="data" :pagination="pageOptions" rowKey="sid">
      ...
      <template slot="ignoreTag" slot-scope="text, record">
        ...
        <div @click="voice(record)" style="position: relative;">
          ...
          <p v-if="record.ignoreTag" style="width: 2px;height: 25px;position: absolute;top: 50%;left: 50%;background: red;transform:rotate(-45deg);margin-top:-12.5px;"></p>
        </div>
      </template>
    ...
    <a-modal centered okText="保存" cancelText="关闭" :maskClosable="false" v-model="visible" @ok="handleOk">
      ...
      <a-form :form="formModel" v-if="editData || type == 1">
          ...
          <a-col span="24">
            <a-form-item label="部件属性：" :label-col="{ span: 6 }" :wrapper-col="{ span: 17 }">
              <a-select
                ...
                @change="handleChange"
              >
                <a-select-option :key="propertyData.id" :value="propertyData.id" v-for="propertyData in alarmSetpropertyData">{{ propertyData.desc }}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
        </a-row>
          ...
          <a-col span="24">
            <a-form-item label="人工复位类型：" :label-col="{ span: 6 }" :wrapper-col="{ span: 17 }">
              <a-select
                ...
                placeholder="请选择"
              >
                <a-select-option :key="artificial.id" :value="artificial.id" v-for="artificial in artificialType">{{ artificial.desc }}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
        </a-row>
        ...
        <a-row :gutter="24">
          <a-col span="24">
            <a-form-item label="生效模式" :label-col="{ span: 6 }" :wrapper-col="{ span: 17 }">
              <a-radio-group
                @change="onChangeRadio"
                v-decorator="[
                  'type',
                  {
                     initialValue: type === 1 ? 1 :editData.alarmRuleVo.type,
                    rules: [{ required: true, message: '请选择生效模式' }]
                  }
                ]"
              >
        ...
        <a-row :gutter="1" v-if="radioValue == 2">
          <a-col span="24">
            <a-form-item label="类名" :label-col="{ span: 6 }" :wrapper-col="{ span: 17 }">
<a-input
            type="text"
            v-decorator="['ruleClass', { initialValue: type === 2 ? editData.alarmRuleVo.ruleClass : '', rules: [{ required: true, message: '请输入类名' }] }]"
          />
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter="1" v-if="variable == 0 && radioValue == 1">
          <a-col span="14">
            <a-form-item label="报警值：" :label-col="{ span: 10 }" :wrapper-col="{ span: 11 }">
              <a-select
                ...
                placeholder="请选择"
              >
                <a-select-option :key="index" :value="contrastData.id" v-for="(contrastData, index) in alarmSetcontrastData">{{ contrastData.desc }}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col span="6">
        ...
        <a-row :gutter="1" v-else-if="variable == 1 && radioValue == 1">
          <a-col span="14">
            <a-form-item label="报警值：" :label-col="{ span: 10 }" :wrapper-col="{ span: 11 }">
              <a-select
                v-decorator="[
...
export default {
  ...
  methods: {
    ...
    refreshApi () {
      // 刷新后台借口
      let this_ = this
      let url = this.baseUrl + this_.apis.alarmApiUrl ;
      request
        .get(url)
        .then(response => {
          this_.$message.success('刷新成功')
        })
        .catch(error => {
          this_.$message.error('刷新失败')
        })
    },
    radioApi () {
      // 单选类型
      let this_ = this
      let url = this.baseUrl + this_.apis.alarmRadio;
      request
        .get(url)
        .then(response => {
          this.radioType=response.data
        })
        .catch(error => {
        })
    },
    ...
    showModal (data) {
      ...
      request
        ...
        .then(response => {
          ...
          this_.radioValue = response.data.alarmRuleVo.type
      ...
      this.formModel.validateFields((err, values) => {
        if (!err) {
          if (this.type == 1) {
            ...
            this.addData.alarmRuleVo.type = values.type
            this.addData.alarmRuleVo.ruleClass = values.ruleClass
            ...
            request
              ...
              .catch(error => {
                this_.$message.error('修改该条报警数据失败')
              })
          }
        }
      })
